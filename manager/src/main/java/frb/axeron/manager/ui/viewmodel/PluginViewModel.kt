...
class PluginViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // 插件兼容选项：开启后识别 Magisk/KernelSU 等未声明 axeronPlugin 的模块
    var isPluginCompatEnabled by mutableStateOf(
        prefs.getBoolean("plugin_compat", false)
    )
        private set

    fun setPluginCompat(enable: Boolean) {
        isPluginCompatEnabled = enable
        prefs.edit { putBoolean("plugin_compat", enable) }
    }
