...
import com.ramcosta.composedestinations.generated.destinations.SelectPatchTargetScreenDestination
 ...
 *
 * Home and Manage both call this, and every path onwards -- an installed app, apks from storage, a
 * re-patch, a loader update, an integrated-mode module edit -- converges on one persisted
 * [PatchRequest] and one screen. The two entry points used to differ: Manage demanded a storage
 * folder first and Home did not, which is why patching from Home failed every time.
 */
fun startNewPatch(navigator: DestinationsNavigator) {
    navigator.navigate(SelectPatchTargetScreenDestination)
}

/** Persists [request] and opens the patch screen on it. */
suspend fun beginPatch(request: PatchRequest, navigator: DestinationsNavigator) {
...
fun newRequestFor(app: AppInfo): PatchRequest =
    PatchRequest(
        ...
        target = PatchTarget.InstalledApp(
 ...
 *
 * The settings it was built with come back with it, so a re-patch is a change to one thing rather
 * than a form filled in again from scratch -- and [modules], when given, replaces the embedded set,
 * which is how an Integrated app's module list is edited at all.
 */
suspend fun rePatchRequestFor(
    ...
    val embedded = recovered.embeddedModules.mapNotNull { binding ->
        binding.apkPath?.let { ModuleRef(binding.packageName, it, binding.origin) }
    }
    return PatchRequest(
        token = UUID.randomUUID().toString(),
        target = PatchTarget.RecoveredOrigin(
            packageName = app.app.packageName,
            label = app.label,
